...
echo "DILIGENT_SANITIZER=$INPUT_SANITIZER" >> $GITHUB_ENV
...
elif [[ "$DILIGENT_TARGET_PLATFORM" == "Web" ]]; then
...
# Add visionOS - specific options
elif [[ "$DILIGENT_TARGET_PLATFORM" == "visionOS" ]]; then
    if [[ "$INPUT_OSX_ARCHITECTURES" == "" ]]; then
        echo "INPUT_OSX_ARCHITECTURES must not be empty"
        exit 1
    fi
    CMAKE_CMD="$CMAKE_CMD -DCMAKE_SYSTEM_NAME=$DILIGENT_TARGET_PLATFORM -DCMAKE_OSX_ARCHITECTURES=$INPUT_OSX_ARCHITECTURES"
fi

if [[ "$DILIGENT_TARGET_PLATFORM" == "Web" ]]; then
